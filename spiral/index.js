...
spiralTraversal = function (grid, top, left) {
...
process.stdin.on('end', function () {
  var input = data.trim().split(' '),
      grid  = generateGrid(+input[1], +input[0]),
      start = input.slice(2, 4).map(parseFloat);

  console.log(spiralTraversal.apply(null, [grid].concat(start)).join(' '));
});
